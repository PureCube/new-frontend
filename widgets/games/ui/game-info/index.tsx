...
export const GameInfo = ({cover, about, title, developer, platform, genre, status, network, engine, download}: gameModelProps) => {
	return (
		<section className="flex flex-col gap-5">
			<h3 className="text-xl md:text-2xl font-heading-semibold uppercase">{title}</h3>
			...
			<div className="flex flex-col md:flex-row gap-20 md:gap-5">
				...
				<div className="md:w-4/12 space-y-3 md:space-y-5">
					<ul className="bg-neutral-900 p-3 md:p-5 rounded-3xl space-y-5 font-body-semibold uppercase text-base md:text-lg">
						...
						{download && download.hasOwnProperty("appstore") && download.hasOwnProperty("googleplay") &&
						<li className="flex gap-3 md:gap-5">
							<a href={download.appstore} target="_blank" rel="noreferrer"><img src="/icons/appstore.svg" alt="link" /></a>
							<a href={download.googleplay} target="_blank" rel="noreferrer"><img src="/icons/googleplay.svg" alt="link" /></a>
						</li>
						}
					</ul>
				</div>
